    ...
    public PossederView() {
        ...
        for (Posseder possession : possessions) {
            ...
            modifyButton.addActionListener(e -> {
                ...
                // Récupérer les anciennes valeurs directement depuis l'objet sélectionné
                String oldPrenom = possession.getPrenom();
                String oldNom = possession.getNom();
                String oldMatricule = possession.getMatricule();
            
                // Récupération des nouveaux prénoms et noms disponibles
                Map<String, List<String>> prenomNomMap = controller.getPrenomsAndNoms();
                List<String> prenoms = new ArrayList<>(prenomNomMap.keySet());
            
                // Sélection des nouveaux prénom et nom
                JComboBox<String> newPrenomBox = new JComboBox<>(prenoms.toArray(new String[0]));
                JComboBox<String> newNomBox = new JComboBox<>();
            
                // Remplissage initial du champ nom
                if (!prenoms.isEmpty()) {
                    String selectedPrenom = prenoms.get(0);
                    newNomBox.setModel(new DefaultComboBoxModel<>(prenomNomMap.get(selectedPrenom).toArray(new String[0])));
                }
            
                // Mise à jour des noms en fonction du prénom sélectionné
                newPrenomBox.addActionListener(event -> {
                    String selectedPrenom = (String) newPrenomBox.getSelectedItem();
                    if (selectedPrenom != null) {
                        newNomBox.setModel(new DefaultComboBoxModel<>(prenomNomMap.get(selectedPrenom).toArray(new String[0])));
                    }
                });
            
                // Liste des nouveaux matricules disponibles
                List<String> matricules = controller.getAllMatricules();
                JComboBox<String> newMatriculeBox = new JComboBox<>(matricules.toArray(new String[0]));
                ...
                Object[] message = {
                        "Nouveau prénom du propriétaire :", newPrenomBox,
                        "Nouveau nom du propriétaire :", newNomBox,
                        "Nouveau matricule du véhicule :", newMatriculeBox,
                ...
                if (option == JOptionPane.OK_OPTION) {
                    try {
                        ...
                        String newPrenom = (String) newPrenomBox.getSelectedItem();
                        String newNom = (String) newNomBox.getSelectedItem();
                        String newMatricule = (String) newMatriculeBox.getSelectedItem();
                        Date newDateFin = Date.valueOf(dateFinField.getText());
            
                        // Appel du contrôleur pour modifier la possession
                        controller.updatePossession(newPrenom, newNom, newMatricule, newDateFin, oldPrenom, oldNom, oldMatricule);
